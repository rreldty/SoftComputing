...
def trimf(x, a, b, c):
    """Triangular membership function - handles edge cases when a==b or b==c"""
...
print(f"\n{'='*50}")
print(f"Input: Berat={berat_input} kg, Aktivitas={aktivitas_input} jam/hari")
print(f"Hasil Mamdani: {kalori_result:.2f} kcal")
print(f"{'='*50}")
...
for label, color in zip(berat_levels, colors):
    axes[0].plot(x_berat, [fuzz_berat(x)[label] for x in x_berat], 
                 color=color, label=label, linewidth=2)
axes[0].set_title('Berat Badan (kg)')
...
for label, color in zip(aktivitas_levels, colors):
    axes[1].plot(x_aktivitas, [fuzz_aktivitas(x)[label] for x in x_aktivitas], 
                 color=color, label=label, linewidth=2)
...
for label, color in zip(kalori_levels, colors):
    axes[2].plot(x_kalori, [mf_kalori(x, label) for x in x_kalori], 
                 color=color, label=label, linewidth=2)
